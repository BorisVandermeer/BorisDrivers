...
namespace Protocols{
    ...
    public:
        CRC();
        CRC(TYPE polynomial, TYPE init_remainder, TYPE final_xor_value, bool isMSD = true);
        void build(TYPE polynomial, TYPE init_remainder, TYPE final_xor_value, bool isMSD = true);
        /**
         * Compute the CRC checksum of a binary message block.
         * @para message, 用来计算的数据
         * @para nBytes, 数据的长度
         */
        TYPE crcCompute(const char * message, const unsigned int nBytes) const;
        TYPE crcCompute(const char * message, const unsigned int nBytes, bool reinit = true);
    ...
    template <typename TYPE>
    CRC<TYPE>::CRC(TYPE polynomial, TYPE init_remainder, TYPE final_xor_value, bool isMSD)
    {
        m_width = 8 * sizeof(TYPE);
        m_topbit = 1 << (m_width - 1);
    ...
    template <typename TYPE>
    void CRC<TYPE>::build(TYPE polynomial, TYPE init_remainder, TYPE final_xor_value, bool isMSD)
    {
        m_polynomial = polynomial;
        m_initial_remainder = init_remainder;
        m_final_xor_value = final_xor_value;
    ...
    template <typename TYPE>
    TYPE CRC<TYPE>::crcCompute(const char * message, const unsigned int nBytes) const{
        unsigned int offset;
        unsigned char byte;
    ...
    template <typename TYPE>
    TYPE CRC<TYPE>::crcCompute(const char * message, const unsigned int nBytes, bool reinit){
        unsigned int offset;
        unsigned char byte;
        if(reinit)
        {
        ...
        if(m_isMSD){
            ...
            for( offset = 0; offset < nBytes; offset++)
            {
                byte = m_remainder ^ message[offset];
                m_remainder = crcTable[byte] ^ (m_remainder >> 8);
            }
        }
        /* The final remainder is the CRC result. */
